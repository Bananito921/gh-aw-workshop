...
A stale or missing [lock file](https://github.github.com/gh-aw/reference/glossary/#workflow-lock-file-lockyml) is the leading cause of `model-access-not-configured` failures at this step. Before triggering the workflow, confirm `daily-report-status.md` and `daily-report-status.lock.yml` are both committed on `main`, and that the `permissions:` block matches the billing path you chose in Step 7d.

> [!TIP]
> <details>
> <summary><b>Optional Side Quest:</b> Want the full two-check walkthrough with recompile commands and a billing-path comparison table?</summary>
>
> Work through [Side Quest: Pre-Flight Checks Before Your First Run](side-quest-08-02-preflight-check.md), then come back here.
>
> </details>
